...
async def chat_handler(client, message):
    ...
    if len(message.text) > 100 and not is_admin:
        return await aio_reply(user_id, "> ⚠️ <b>Message limit exceeded!</b>\n> You can only send up to 100 characters (including spaces).", message.id)
    ...
    if len(history) >= 3 and not is_admin:
        return await aio_reply(user_id, "> ⏳ <b>Anti-Spam Active:</b>\n> You sent 3 messages too quickly. Please wait 30 seconds.", message.id)
    ...
    if not bot_config.get('chat_enabled'):
        if not is_admin:
            return await aio_reply(user_id, "> 💬 <b>Global chat is currently OFF.</b>\n> The system is only accepting videos and photos.", message.id)
        else:
            await aio_reply(user_id, "> 💬 <b>System Alert:</b>\n> Global chat is OFF. Regular users will receive a warning instead of their text being forwarded.", message.id)
            
    if user.get('chat_muted_until') and user['chat_muted_until'] > datetime.now():
        return await aio_reply(user_id, f"> 🔇 <b>ACCESS DENIED: You are currently muted.</b>\n> Restriction lifts at: {user['chat_muted_until'].strftime('%H:%M %d/%m')}", message.id)
    ...
    if not is_admin:
        ...
        if has_link or is_forward or re.search(r"(http://|https://|\.com|\.net|\.org|\.me|t\.me|@\w+)", message.text.lower()):
            ...
            return await aio_reply(user_id, f"> 🚨 <b>SECURITY VIOLATION: UNAUTHORIZED LINK OR FORWARD DETECTED!</b>\n> Your account has been temporarily muted for {config.Config.MUTE_PENALTY_MINUTES} minutes.", message.id)
