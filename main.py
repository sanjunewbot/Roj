...
async def health_check(request): return web.Response(text="Bot is ALIVE and RUNNING!")
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(Config.PING_URL) as response: pass
        except: pass
        await asyncio.sleep(5 * 60)
async def run_2h_reminders():
    inactive_users = await db.get_users_to_remind()
    reminder_text = ("⚠️ <b>REMINDER: AAPKA TIME 0 HO CHUKA HAI!</b>\n\nKripya koi media send karein, har video/media ke hisaab se aapka time <b>30 minutes</b> badh jayega.\n\nMedia send karte hi aapka access turant chalu ho jayega! 🚀")
...
async def expiry_check(): await db.remove_expired_premium()
async def main():
    ...
    for admin_id in Config.ADMIN_IDS:
        try: await bot.send_message(chat_id=admin_id, text=restart_text)
        except Exception as e: print(f"Could not send restart alert to Admin {admin_id}: {e}")
    await bot.set_bot_commands([
        ...
        BotCommand("rem_prem", "✂️ Admin: Remove Premium (#Nickname)"),
        ...
        BotCommand("wait", "🚦 Admin: Toggle Registrations"),
        BotCommand("broadcast", "📢 Admin: Mass Message Broadcast"),
        BotCommand("chat", "💬 Admin: Toggle Global Chat (on/off)")
    ])
...
if __name__ == "__main__":
    ...
    except KeyboardInterrupt: print("🛑 System Stopped.")
