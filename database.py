...
class db:
    ...
    @staticmethod
    async def get_active_users(): return await users.find({"is_banned": {"$ne": True}, "$or": [{"is_premium": True}, {"active_until": {"$gt": datetime.now()}}]}).to_list(length=None)
    @staticmethod
    async def get_stats():
    ...
    async def get_bot_settings():
        ...
        if not s:
            default = {"id": "global_config", "bin_channel": None, "pm_dlt": False, "dlt_time": 60, "ref_system": False, "ref_count": 5, "ref_text": "Invite!", "ref_time_str": "7d", "registration_open": True, "media_restriction": False, "chat_enabled": False}
